...
begin
  ...
  if PDX_Remot_Set_Comm(PChar(ComPort), 9600, 8, 0, 0) = 0 then raise Exception.Create('Errore ' + IntTostr(PDX_Get_Last_Error) + ' nell''impostazione dei parametri della porta.');
  ...
  if PDX_Upload(PChar(IntToStr(FormNumber))) = 0 then raise Exception.Create('Errore ' + IntTostr(PDX_Get_Last_Error) + ' non è stato possibile entrare in modalità UPLOAD.');
  ...
  if PDX_Upload_Deletedata(PChar(IntToStr(FormNumber))) = 0 then raise Exception.Create('Errore ' + IntTostr(PDX_Get_Last_Error) + ' non è stato possibile cancellare i dati dal terminale.');
...
begin
  if PDX_Get_Data('RX', Buffer) = 0 then raise Exception.Create('Errore ' + IntTostr(PDX_Get_Last_Error) + ' comando Get_Data.');
