...
begin
  ...
  begin
    ...
    // Use SEQUENCE with DEFAULT for automatic ID assignment (Firebird 3.0+, allows RETURNING clause to work)
    else if ATable.UsesSequenceForKeyGeneration then
      Result := Format('%s INTEGER DEFAULT NEXT VALUE FOR %s NOT NULL', [AField.SqlFieldName, ATable.GetSequenceName])
    else
