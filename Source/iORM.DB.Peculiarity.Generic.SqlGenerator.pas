...
unit iORM.DB.Peculiarity.Generic.SqlGenerator;
...
type
  ...
  TioSqlGeneratorGeneric = class(TioSqlGenerator)
  public
...
// Divided in two parts for MSSqlServer or similar.
    class procedure GenerateSqlInsertFirstPart(const AQuery:IioQuery; const AContext:IioContext); //&&&&
    class procedure GenerateSqlInsertLastPart(const AQuery:IioQuery; const AContext:IioContext); //&&&&
// Passare per il Connection Manager o fare in modo che abbia tramite parametri il DbPeculiarity???
    class function HasAutoIncrementIDRetrieveWithInsertQuery(AContext:IioContext):Boolean;
  end;

  TioSqlGeneratorStructured = class(TioSqlGeneratorGeneric)
  private
    class procedure GenerateSqlForExists(const AQuery:IioQuery; const AContext:IioContext); override;
    class procedure GenerateSqlForCreateIndex(const AQuery:IioQuery; const AContext:IioContext; AIndexName:String; const ACommaSepFieldList:String; const AIndexOrientation:TioIndexOrientation; const AUnique:Boolean); override;
    class procedure GenerateSqlForDropIndex(const AQuery:IioQuery; const AContext:IioContext; AIndexName:String); override;
...
class procedure TioSqlGeneratorGeneric.GenerateSqlLastIDAfterInsert(
  const AQuery: IioQuery; const AContext: IioContext);
begin
  //No default query
end;

class procedure TioSqlGeneratorGeneric.GenerateSqlNextIDBeforeInsert(
  const AQuery: IioQuery; const AContext: IioContext);
begin
  //No default query
end;
...
class function TioSqlGeneratorGeneric.HasAutoIncrementIDRetrieveWithInsertQuery(
  AContext: IioContext): Boolean;
begin
// Passare per il Connection Manager o fare in modo che abbia tramite parametri il DbPeculiarity???
  Result:=TioConnectionManager.GetConnectionInfo.DbPeculiarity.HasAutoIncrementIDRetrieveWithInsertQuery(AContext.GetTable);
end;

class procedure TioSqlGeneratorGeneric.GenerateSqlInsertFirstPart(
...
class procedure TioSqlGeneratorGeneric.GenerateSqlInsertLastPart(
...
{ TioSqlGeneratorStructured }


class procedure TioSqlGeneratorStructured.GenerateSqlForCreateIndex(
  const AQuery: IioQuery; const AContext:IioContext; AIndexName:String;
  const ACommaSepFieldList: String; const AIndexOrientation: TioIndexOrientation;
  const AUnique: Boolean);
var
...
class procedure TioSqlGeneratorStructured.GenerateSqlForDropIndex(
  const AQuery: IioQuery; const AContext:IioContext; AIndexName: String);
begin
  // Index Name
  AIndexName := TioSqlTranslator.Translate(AIndexName, AContext.GetClassRef.ClassName, False);
...
class procedure TioSqlGeneratorStructured.GenerateSqlForExists(const AQuery: IioQuery; const AContext: IioContext);
begin
// No inherited
//  inherited;
  // Build the query text
