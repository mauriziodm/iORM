...
// TODO: Probabilmente anche qui dovrei usare una TransactionCollection (come per LoadList e PersistList ad esempio) perch� pu� riguardare pi� oggetti mappati anche su connessioni diverse
...
begin
  ...
  // -----------------------------------------------------------
  // FETCH ID BEFORE INSERT (using with sequence/generator)
  //
  // For tables using Sequence for ID generation (Firebird default), we need to get the ID
  // BEFORE the INSERT (ID pre-fetch) because there is no automatic ID generation in the INSERT itself.
  // The ID MUST always be assigned (if null), otherwise the object would persist with ID zero.
  // This approach ensures compatibility with all Firebird versions (2.0+).
  //
  // FUTURE OPTIMIZATION: Firebird 3.0+ supports "DEFAULT NEXT VALUE FOR <sequence>" in column
  // definitions, which allows using "INSERT ... RETURNING ID" even for sequences (like Identity).
  // This would unify the approach across all databases and eliminate this pre-fetch query.
  // However, this requires knowing the Firebird version at runtime, which is currently only
  // available in DBBuilder context, not in the normal query execution context.
  ...
  // -----------------------------------------------------------
  // FETCH ID AFTER INSERT (using with identity)
  //
  ...
  LUseReturningClause := (AContext.GetTable.GetKeyGenerationStrategy <> kgsSequence) and
  ...
  // If using Identity strategy, append RETURNING clause and execute as query that returns data
  if LUseReturningClause then
  begin
  ...
  begin
    // No RETURNING clause needed (ID already set or Sequence strategy used), execute as normal INSERT
    LQuery.ExecSQL;
  end;
  // -----------------------------------------------------------
...
begin
  ...
  try
    ...
    if LContext.ActionType > atDoNotPersist then
    begin
      // access-token validation
      // note: The anonymous method of type "TioTokenValidateMethod" can return true or false
      //        to authorize or not the operation to be performed. But be careful, if the operation
...
begin
  ...
  begin
    ...
    case LMasterProp.GetRelationType of
      ...
      // sul DB senza passare dagli oggetti (per� questo salterebbe ETM ad esempio); oppure ancora fare prima un reload
      // di tutti gli oggetti child e poi eliminarli uno ad uno come oggetti, questo risolverebbe tutto per� con
      ...
      begin
        ...
        // NB: richiamo "io._ExecutePSRequest" perch� in questo modo usa la persistence strategy corretta nel caso
