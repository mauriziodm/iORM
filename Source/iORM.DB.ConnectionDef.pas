...
type
  ...
  strict private
    FEnabled: Boolean;
    FForeignKeys: TioDBBuilderMode;
    FIndexes: TioDBBuilderMode;
  published
    constructor Create;
    // Properties
    // NB: DBBuilder related events are on ConnectionDef component, not in this class
    property Enabled: Boolean read FEnabled write FEnabled default False;
    property Indexes: TioDBBuilderMode read FIndexes write FIndexes default ifmEnabled;
    property ForeignKeys: TioDBBuilderMode read FForeignKeys write FForeignKeys default ifmEnabled;
  end;
...
// TODO: Non ricordo se la propriet� IioSynchroStrategy_Client � da eliminare, controllare
...
begin
  ...
  begin
    try
      LContext.Execute(AForce);

      if Assigned(FAfterDBBuild) then
        FAfterDBBuild(Self, LContext.Status, LContext.Lines, LContext.Warnings.Lines);
    except
      on E: Exception do
      begin
        if Assigned(FOnDBBuildException) then
          FOnDBBuildException(Self, LContext.Status, LContext.Lines, LContext.Warnings.Lines, E, LReRaise);
