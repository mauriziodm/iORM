...
procedure TioDataSetCustom.DoAfterReceiveSelection(var ASelected: TObject; var ASelectionType: TioSelectionType);
begin
  if Assigned(FAfterReceiveSelectionObject) then
    FAfterReceiveSelectionObject(Self, ASelected, ASelectionType);
end;
...
procedure TioDataSetCustom.DoAfterReceiveSelection(var ASelected: IInterface; var ASelectionType: TioSelectionType);
begin
  if Assigned(FAfterReceiveSelectionInterface) then
    FAfterReceiveSelectionInterface(Self, ASelected, ASelectionType);
end;
...
begin
  if Assigned(FonReceiveSelectionObject) then
    FonReceiveSelectionObject(Self, ASelected, ASelectionType, ADone);
end;

procedure TioDataSetCustom.DoReceiveSelection(var ASelected: IInterface; var ASelectionType: TioSelectionType; var ADone: Boolean);
begin
  if Assigned(FonReceiveSelectionInterface) then
    FonReceiveSelectionInterface(Self, ASelected, ASelectionType, ADone);
end;

function TioDataSetCustom.FirstMasterPersistenceBindSource: IioBindSource;
begin
...
function TioDataSetCustom.GetMasterBindSource: IioBindSource;
begin
...
function TioDataSetCustom.HasMasterBS: Boolean;
begin
...
begin
  ...
  if IsDetailBS and not MasterPropertyName.IsEmpty then
    SetActiveBindSourceAdapter(TioLiveBindingsFactory.GetDetailBSAfromMasterBindSource(nil, Self, MasterBindSource, MasterPropertyName))
  else if IsFromBSLoadType or (IsDetailBS and MasterPropertyName.IsEmpty) then
    SetActiveBindSourceAdapter(TioLiveBindingsFactory.GetNaturalBSAfromMasterBindSource(nil, Self, MasterBindSource))
  else
  begin
    SetActiveBindSourceAdapter(TioLiveBindingsFactory.GetBSA(nil, Self, ADataObject, AOwnsObject));
...
procedure TioDataSetCustom._Action_Append(const ARaiseIfSaved, ARaiseIfChangesExists: Boolean);
begin
  if IsMasterBS then
...
procedure TioDataSetCustom._Action_AppendIntf(AObject: IInterface; const ARaiseIfSaved, ARaiseIfChangesExists: Boolean);
begin
  if IsMasterBS then
...
procedure TioDataSetCustom._Action_AppendObj(AObject: TObject; const ARaiseIfSaved, ARaiseIfChangesExists: Boolean);
begin
  if IsMasterBS then
    (Self as IioMasterBindSource).Persistence.Append(AObject, ARaiseIfSaved, ARaiseIfChangesExists)
  else
...
procedure TioDataSetCustom._Action_Delete(const ARaiseIfSaved, ARaiseIfChangesExists: Boolean);
begin
  if IsMasterBS then
...
procedure TioDataSetCustom._Action_Insert(const ARaiseIfSaved, ARaiseIfChangesExists: Boolean);
begin
  if IsMasterBS then
...
procedure TioDataSetCustom._Action_InsertIntf(AObject: IInterface; const ARaiseIfSaved, ARaiseIfChangesExists: Boolean);
begin
  if IsMasterBS then
...
procedure TioDataSetCustom._Action_InsertObj(AObject: TObject; const ARaiseIfSaved, ARaiseIfChangesExists: Boolean);
begin
  if IsMasterBS then
    (Self as IioMasterBindSource).Persistence.Insert(AObject, ARaiseIfSaved, ARaiseIfChangesExists)
  else
