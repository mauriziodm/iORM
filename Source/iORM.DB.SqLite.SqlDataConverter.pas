    ...
    tkEnumeration:
      begin
        if AQuery.Fields.FieldByName(AProperty.GetSqlFieldAlias) is TBooleanField then
          ...
          Result := TValue.FromOrdinal(AProperty.GetRttiType.Handle, // This is the PTypeInfo of the PropertyType
...
class procedure TioSqlDataConverterSqLite.SetQueryParamByContext(const AQuery: IioQuery; const AProp: IioProperty; const AContext: IioContext);
begin
  inherited;
  ...
  else if AProp.isHasManyChildVirtualProperty then
  ...
  // ATTENZIONE!!! - Ho modificato il codice come sopra, in pratica facendo in modo che le propriet� di
  // tipo TDateTime, TDate e TTime vengano sempre impostate nel parametro della query sempre come TDate
  // perch� nel progetto Marpimar di Omar e Thomas soccedeva che una propriet di tipo DateTime non veniva
  // persistita bene, in pratica persisteva sempre uno zero. Facendo cos� invece va bene.
  // Non abbiamo capito il perch�.
  // ==================== OLD CODE =================================
  // // If the property is of type TDateTime or TDate or TTime and the value is equal to
  // //  zero then set che ParamValue to NULL
  // if(   (AProp.GetTypeInfo = System.TypeInfo(TDateTime)) or (AProp.GetTypeInfo = System.TypeInfo(TDate)) or (AProp.GetTypeInfo = System.TypeInfo(TTime))   )
...
begin
  inherited;
  // Default
  Result := ''; // Mauri 13/06/20223: Nested where property (prima era 'NULL')
