...
type
  ...
  public
    ...
    class procedure Notify(const Sender: TObject; const AActiveBindSourceAdapter: IioActiveBindSourceAdapter; const [Ref] ANotification: TioBSNotification;
      const AEnableNotificationToSourceAdapterForNaturalBSA: Boolean = True);
...
class procedure TioCommonBSABehavior.Notify(const Sender: TObject; const AActiveBindSourceAdapter: IioActiveBindSourceAdapter;
  const [Ref] ANotification: TioBSNotification; const AEnableNotificationToSourceAdapterForNaturalBSA: Boolean = True);
begin
  ...
  if ANotification.DirectionRoot and AEnableNotificationToSourceAdapterForNaturalBSA and Supports(AActiveBindSourceAdapter, IioNaturalActiveBindSourceAdapter)
  then
...
begin
  TioContextFactory.Context(itRegular, AActiveBindSourceAdapter.Current.ClassName, nil, AActiveBindSourceAdapter.Current, nil, '', '', BL_DEFAULT, nil).ObjStatus := AObjStatus;
end;

class function TioCommonBSABehavior.UseObjStatus(const AActiveBindSourceAdapter: IioActiveBindSourceAdapter): Boolean;
begin
  Result := TioContextFactory.Context(itRegular, AActiveBindSourceAdapter.Current.ClassName, nil, AActiveBindSourceAdapter.Current, nil, '', '', BL_DEFAULT, nil)
...
{$ENDIF}
          case LProperty.PropertyType.TypeKind of
            tkEnumeration:
              ...
              if SameText(LTypeName, 'boolean') or SameText(LTypeName, 'bool') then // 'bool' for C++
              ...
              else if TioEnumContainer._Contains(TRttiEnumerationType(LProperty.PropertyType)) then
                ...
                case LProperty.PropertyType.TypeSize of
                  1:
                    LCollectionEditorField := CreateRttiPropertyField<UInt8>(LProperty, ABindSourceAdapter, AGetMemberObject, mtUInteger,
                      APath + LProperty.Name);
                  2:
                    LCollectionEditorField := CreateRttiPropertyField<UInt16>(LProperty, ABindSourceAdapter, AGetMemberObject, mtUInteger,
                      APath + LProperty.Name);
                  4:
                    LCollectionEditorField := CreateRttiPropertyField<UInt32>(LProperty, ABindSourceAdapter, AGetMemberObject, mtUInteger,
                      APath + LProperty.Name);
                end;
            tkInteger:
              begin
                case LTypeData.OrdType of
                  ...
                  otSWord: // Int16
                    LCollectionEditorField := CreateRttiPropertyField<Int16>(LProperty, ABindSourceAdapter, AGetMemberObject, mtInteger,
                      APath + LProperty.Name);
                  otSLong: // Int32
                  ...
                  LCollectionEditorField := CreateRttiPropertyField<Integer>(LProperty, ABindSourceAdapter, AGetMemberObject, mtInteger,
                    APath + LProperty.Name);
                end
              end;
            tkWChar:
...
{$ENDIF}
            tkFloat:
              case LTypeData^.FloatType of
                ...
                System.TypInfo.ftDouble:
                  begin
                    if LTypeInfo = System.TypeInfo(TDate) then
                      ...
                      LCollectionEditorField := CreateRttiPropertyField<TDateTime>(LProperty, ABindSourceAdapter, AGetMemberObject, mtDateTime,
                        APath + LProperty.Name)
                    else
                      LCollectionEditorField := CreateRttiPropertyField<double>(LProperty, ABindSourceAdapter, AGetMemberObject, mtFloat,
                        APath + LProperty.Name);
                  end;
                ftExtended:
                ...
                ftCurr:
                  LCollectionEditorField := CreateRttiPropertyField<currency>(LProperty, ABindSourceAdapter, AGetMemberObject, mtCurrency,
                    APath + LProperty.Name);
...
class procedure TioCommonBSABehavior.DeleteListViewItem(const AActiveBindSourceAdapter: IioActiveBindSourceAdapter;
