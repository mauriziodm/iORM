...
type
  ...
  public
    ...
    /// </summary>
    procedure CreateOrUpdateDB(const Force: Boolean = False; const AScript: IioDBBuilderScript = nil); overload;
    /// <summary>
...
procedure TioDBBuilderEngine.BuildCreateDBScript(const AScript: IioDBBuilderScript);
begin
  if not Assigned(AScript) then
    raise EioInvalidArgumentException.Create(ClassName, 'BuildCreateDBScript', 'AScript is not assigned.');
...
procedure TioDBBuilderEngine.BuildCreateOrUpdateDBScript(const AScript: IioDBBuilderScript);
begin
  if not Assigned(AScript) then
    raise EioInvalidArgumentException.Create(ClassName, 'BuildCreateOrUpdateDBScript', 'AScript is not assigned.');

  if not Analyzed then
    raise EioDBBuilderException.Create(ClassName, 'BuildCreateOrUpdateDBScript', 'Unable to build SQL script: schema not analyzed');
...
procedure TioDBBuilderEngine.CreateOrUpdateDB(const Force: Boolean; const AScript: IioDBBuilderScript);
var
...
begin
  ...
  begin
    ...
    if LBuildScript then
      LScript := TioDBBuilderFactory.NewScript(FConnectionDefName)
    else
      LScript := AScript;
    ...
    if LBuildScript then
      BuildCreateOrUpdateDBScript(LScript);
...
function TioDBBuilderEngine_New.GetScript: IioDBBuilderScript;
begin
  Result := FScript;
end;
