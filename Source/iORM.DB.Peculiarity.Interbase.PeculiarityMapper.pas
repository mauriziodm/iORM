//&&&&&
unit iORM.DB.Peculiarity.Interbase.PeculiarityMapper;
...
Type
  TioInterbaseConnectionManager = Class(TioConnectionManager)
    class function NewInterbaseConnectionDef(const AServer, ADatabase, AUserName, APassword, ACharSet: String;
  ...
  // Class for Interbase connection
  TioInterbaseConnectionDef = class(TioCustomConnectionDef)
  public
...
Type
  TioDbPeculiarityInterbase = class(TioDbPeculiarityGeneric)
  public
...
(* TioInterbaseConnectionManager *)

class function TioInterbaseConnectionManager.NewInterbaseConnectionDef(const AServer, ADatabase, AUserName, APassword, ACharSet: String;
  const AAsDefault:Boolean=True; const APersistent:Boolean=False;
  const APooled:Boolean=False; const AConnectionName: String=IO_CONNECTIONDEF_DEFAULTNAME): IIoConnectionDef;
begin
  Result := Self.NewCustomConnectionDef(AConnectionName, APooled, AAsDefault);
  Result.Params.DriverID := 'IB';
  ...
  FConnectionManagerContainer.AddOrSetValue(AConnectionName, TioConnectionInfo.Create(AConnectionName(*&&&&, cdtFirebird*), TioDbPeculiarityInterbase, APersistent));
end;

{ TioInterbaseConnectionDef }

constructor TioInterbaseConnectionDef.Create(AOwner: TComponent);
begin
  inherited;
  Port := 3050;
end;

procedure TioInterbaseConnectionDef.RegisterConnectionDef;
begin
  ConnectionDef := TioInterbaseConnectionManager.NewInterbaseConnectionDef(Server,
...
{ TioDbPeculiarityInterbase }

class function TioDbPeculiarityInterbase.SqlDataConverterRef: TioSqlDataConverterRef;
begin
//&&&&  Result:=TioSqlDataConverterFirebird;
  Result:=TioSqlDataConverterStructured;
end;

class function TioDbPeculiarityInterbase.SqlGeneratorRef: TioSqlGeneratorRef;
begin
  Result:=TioSqlGeneratorFirebird;
end;

class Function TioDbPeculiarityInterbase.HasSqlNextIDBeforeInsert(Const AContextTable:IioContextTable):Boolean;
Begin
  Result:=True;
End;
