...
{$REGION '===== ETM ATTRIBUTES & TIMESLOT ====='}
  ...
  // NB: Per la nuova gestione dei conflitti servir� la possibilit� di memorizzare due data e ora, una che sar� il momento
  // quando � stata fatto l'update sul DB remoto (es: mobile) e l'altra la data e ora di quanto � stato fatto l'update
  ...
  private
    ...
    // NB: Questo � un anonymous method che viene passato dal BindSource che sta esponendo il TimeSlot stesso e che permette
    // di risalire alla versione corrente della entit� attraverso la catena "ETMBindSource.etmFor.Current"
    [ioSkip]
  ...
  etmPropertyAttribute = class(TCustomAttribute) // NB: Lasciarlo con "Attribute" alla fine del nome della classe cos� � pi� chiaro nelle eventuali exceptions
  strict private
...
uses
  iORM, iORM.Utilities, iORM.Exceptions, iORM.ETM.Engine,
...
begin
  ...
  // Se l'entit� non ha un ID valido non � possibile che l'ETM funzioni
