...
function TioDBBuilderSqLiteSqlGenerator.CreateField(const AProperty: IioContextProperty; const AAddNullableSyntax: Boolean = True;
  const AAddType: Boolean = False): String;
var
  LFieldName: string;
  LFieldType: string;
...
function TioDBBuilderSqLiteSqlGenerator.AddForeignKey(const ASourceTableName: String; const ASourceFieldName: String;
...
function TioDBBuilderSqLiteSqlGenerator.AddForeignKeyInCreate(const ABuilderTable: IioDBBuilderTable): String;
var
...
begin
  ...
  begin
    // Mauri 09/07/2020: Per risolvere problemi con foreign keys (DEFERRABLE INITIALLY DEFERRED)
    Result := Result + ' ' + Format('FOREIGN KEY("%s") REFERENCES %s("%s") DEFERRABLE INITIALLY DEFERRED',
      [LFk.SourceFieldName, LFk.DestinationTableName, LFk.DestinationFieldName]) + ',';
  end;
...
function TioDBBuilderSqLiteSqlGenerator.AddSequences(const ATableName: String; const AIDProperty: IioContextProperty): String;
begin
  // Do Nothing
end;

function TioDBBuilderSqLiteSqlGenerator.AddIndex(const AContext: IioContext; const AIndexName, ACommaSepFieldList: String;
  const AIndexOrientation: TioIndexOrientation; const AUnique: Boolean): String;
var
  LQuery: IioQuery;
begin
...
function TioDBBuilderSqLiteSqlGenerator.BeginCreateTable(const ATableName: String): String;
begin
  FCreateTableScript := '';
...
function TioDBBuilderSqLiteSqlGenerator.ConversionTypeAdmitted(AColumnType, AModelColumnType: string): Boolean;
var
...
begin
  ...
  begin
    if not ADestTable.FieldExists(AField.FieldName) then
      Continue;
    if not AField.IsSqlField then
      Continue;
...
function TioDBBuilderSqLiteSqlGenerator.FieldModified(const ADbName: String; const ATableName: String;
...
function TioDBBuilderSqLiteSqlGenerator.GetRemark(const AWarnings: Boolean): String;
begin
  if AWarnings then
    Result := '-- '
  else
...
function TioDBBuilderSqLiteSqlGenerator.RemoveLastComma(const AValue: string): string;
begin
  if AValue.EndsWith(',') then
    Result := AValue.Substring(0, AValue.Length - 1);
end;

function TioDBBuilderSqLiteSqlGenerator.RestructureTable(const ATableList: TioDBBuilderTableList): String;
var
  LQueryDrop: IioQuery;
...
begin
  ...
  begin
    ...
    LQueryDrop.SQL.Add('PRAGMA defer_foreign_keys=off;'); // Mauri 09/07/2020: Per risolvere problemi con foreign keys
    ...
    LQueryDrop.SQL.Add('PRAGMA defer_foreign_keys=on;'); // Mauri 09/07/2020: Per risolvere problemi con foreign keys
  end;
...
begin
  ...
  LQuery.Open;
  Result := LQuery.Fields.FieldByName('TableExists').AsInteger > 0;
  LQuery.Close;
end;

function TioDBBuilderSqLiteSqlGenerator.UseDatabase(const ADbName: string): String;
begin
  // Do Nothing
  Result := '';
end;
